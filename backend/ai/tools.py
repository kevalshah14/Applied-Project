...
try:
    ...
    # Check if model exists in expected location, if not rely on ultralytics or move it
    if not os.path.exists(model_path):
        if os.path.exists("sam2.1_b.pt"):
             os.rename("sam2.1_b.pt", model_path)
             
    sam_model = SAM(model_path if os.path.exists(model_path) else "sam2.1_b.pt")
    
    # If it was downloaded to root by the fallback
...
async def _pickup_object_async(object_description: str, x_mm: float = None, y_mm: float = None, z_mm: float = None) -> str:
    ...
    approach_z = z_target 
    print(f"DEBUG: Moving to approach position (z={approach_z:.3f}mm)")
    ...
    print(f"DEBUG: Moving down to object (z={z_target:.3f}mm)")
    ...
    print("DEBUG: Closing gripper to grasp")
    ...
    lift_z = z_target + 100.0
    print(f"DEBUG: Lifting object (z={lift_z:.3f}mm)")
...
async def _place_object_async(object_description: str, x_mm: float = None, y_mm: float = None, z_mm: float = None) -> str:
    ...
    final_message = f"✓ Successfully placed {object_description}!\n\n{message}"
